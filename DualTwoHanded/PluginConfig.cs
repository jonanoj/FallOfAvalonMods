...
public class PluginConfig
{
    ...
    public ConfigEntry<bool> AllowOneHandedTools { get; private set; }
    ...
    public PluginConfig(ConfigFile config)
    {
        ...
        try
        {
            const string twoHanded = "TwoHanded";
            ...
            RequiredStrengthMultiplier = config.Bind(twoHanded, nameof(RequiredStrengthMultiplier), 2f,
                ...
                """);
            AllowOneHandedTools = config.Bind(twoHanded, nameof(AllowOneHandedTools), false,
                "When set to true, two-handed tools (i.e. pickaxe, fishing rod, shovel, etc.) can be equipped in one hand.");
        }
        finally
        {
            config.Save();
